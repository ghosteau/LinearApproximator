...
def linearize(user_function, a):
    x = init_symbol("x")
    user_function = init_function(user_function)
    a = float(a)

    if a not in domain(user_function):
    ...
    linearized_function = init_function(f"{foa} + {fpoa} * (x - {a})")
...
linearized_function = linearize(user_function, a)
